...
def twitter(pk, title, resumen):
    ...
    api = tweepy.API(auth)
    api.update_status("%s\n%s...\n%s\n#cfbc" % (title, resumen, "https://bartolo.org/noticia/%s/?utm_source=twitter-tweet&utm_medium=twitter&utm_campaign=crecimiento" % pk))

@background(schedule=5)
def facebook(pk, title, resumen):
    ...
    graph.put_object(
        ...
        link="https://bartolo.org/noticia/%s/?utm_source=facebook-posts&utm_medium=facebook&utm_campaign=crecimiento" % pk
    )
